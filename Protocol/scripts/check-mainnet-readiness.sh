...
grep -q 'SUBMIT_MAINNET_SERVER_RANDOMNESS' scripts/preview-mainnet-server-randomness-switch.mjs
...
grep -q 'sendRawTransaction' scripts/preview-mainnet-server-randomness-switch.mjs
grep -q 'switchedConfig?.paused, true' scripts/preview-mainnet-server-randomness-switch.mjs
